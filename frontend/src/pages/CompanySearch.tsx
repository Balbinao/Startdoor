...
export const CompanySearch = observer(() => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        setIsLoading(true);
        await Promise.all([getCompanies(), getSectors()]);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <div className="flex w-full items-center justify-center py-20">
        <span className="text-(--grey-300)">{MESSAGES_LOADING.GET}</span>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-4">
          <div className="flex gap-3">
            ...
            <button
              ...
              className={'flex h-12 w-12 items-center justify-center rounded-xl border transition-colors bg-(--grey-900) border-(--grey-600) text-(--grey-100)'}
            >
